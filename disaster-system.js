...
/* 最终营业流程：候选客先在地图出现，抽中者走到店门后才开始接待。 */
openShopForPeriod=async function(){ensureFortressState();const totals=fortressTotals();if(totals.health<=0){state.history.unshift({day:state.day,story:"你试着推开小店，却发现整间铺子已经无法安全营业。",outcome:"坚固度为 0 · 营业失败"});finishEmptyPeriod("小店已经完全损坏。修好至少一个部件后才能重新营业。");return}state.actionInProgress=true;const quietReason=chooseCustomer();if(!Number.isInteger(state.currentCustomer)){state.actionInProgress=false;finishEmptyPeriod(`${quietReason||"本时段没有脚步停在门前"}。`);return}const visitor=dailyCustomer();if(typeof walkVisitorToShop==="function")await walkVisitorToShop(visitor);state.actionInProgress=false;const blocked=blockedPartForVisitor(visitor);if(blocked){markCurrentMet();relation(visitor.name).visits+=1;state.history.unshift({day:state.day,story:`${visitor.name}沿路来到门前，看了看损坏的${SHOP_PARTS[blocked].name}后离开了。`,outcome:`${SHOP_PARTS[blocked].name}损坏 · 无法交易`});state.periodResult={icon:"🛠️",title:`${visitor.name}没有进店`,text:`${SHOP_PARTS[blocked].name}已经彻底损坏，客人看了一眼便离开了。`};state.dailyActions.push({time:currentTime().name,icon:"🛠️",type:"shop",title:`${visitor.name}到访`,outcome:`${SHOP_PARTS[blocked].name}损坏，未能交易`});advancePeriod(true);return}state.visitorRevealed=true;state.resolved=false;markCurrentMet();if(state.currentVisitType==="shop")discoverCurrentRequest();state.pendingPeriodAction={time:currentTime().name,icon:"🔔",type:"shop",visitor:visitor.name,title:`小店营业 · ${visitor.name}`};state.periodResult={icon:"🔔",title:`${currentTime().name}开门营业`,text:`${visitor.name}沿着小镇道路来到店门前，门铃随即响起。`};save();render();document.querySelector("#visitorCounter").scrollIntoView({behavior:"smooth",block:"center"})};
