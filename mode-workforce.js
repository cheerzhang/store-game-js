...
const GAME_MODES={
  lean:{icon:"⚖️",name:"低地最会经营的小店",note:"完成小团队年度目标，并挑战钱箱最高纪录。",rule:"最多 3 位员工 · 年收入 ≥220 · 坚固度 ≥80% · 无欠薪 · 信用 ≥4"},
...
function ensureModeStats(){state.modeStats||={periodStart:state.day,revenue:0,revenueByVisitor:{},maxEmployees:state.employees.length,disasters:0,strikes:0,rolesUsed:[],victories:[]};state.modeStats.periodStart??=state.day;state.modeStats.revenue??=0;state.modeStats.revenueByVisitor||={};state.modeStats.maxEmployees=Math.max(state.modeStats.maxEmployees||0,state.employees.length);state.modeStats.disasters??=0;state.modeStats.strikes??=0;state.modeStats.rolesUsed||=[];state.modeStats.victories||=[]}
...
function recordModeRevenue(amount,visitorName){if(!state.gameMode||!amount)return;ensureModeStats();state.modeStats.revenue+=amount;if(visitorName)state.modeStats.revenueByVisitor[visitorName]=(state.modeStats.revenueByVisitor[visitorName]||0)+amount;save()}
...
function evaluateAnnualMode(){
  ...
  if(won)modeVictory(`${GAME_MODES[mode].name} · 年度胜利`,`你完成了这一经营模式的年度目标。胜利记录会保留，小店仍可继续进入下一年。`,`<strong>${reason}</strong>`);state.modeStats.periodStart=state.day;state.modeStats.revenue=0;state.modeStats.revenueByVisitor={};state.modeStats.maxEmployees=state.employees.length;state.modeStats.disasters=0;state.modeStats.strikes=0;state.modeStats.rolesUsed=[];save()
}
...
function renderModeUI(){
  const box=document.querySelector("#modeProgress");if(!box)return;if(!state.gameMode){box.innerHTML="";return}ensureModeStats();const mode=GAME_MODES[state.gameMode],days=Math.min(336,state.day-state.modeStats.periodStart+1),extra=state.gameMode==="lean"?`钱箱 ${state.coins} 枚 · 员工峰值 ${state.modeStats.maxEmployees}/3`:state.gameMode==="staff"?`员工 ${state.employees.length}/8 · 平均士气 ${state.employees.length?Math.round(state.employees.reduce((n,e)=>n+ensureEmployeeMorale(e).morale,0)/state.employees.length):0}/70`:state.gameMode==="storm"?`已挺过灾害 ${state.modeStats.disasters}/8 · 风险加成 +18%`:state.gameMode==="story"?`隐藏来客 ${SPECIAL_NAMES.filter(name=>state.metCustomers.includes(name)).length}/2`:"不设年度胜利与强制失败";box.innerHTML=`<span>${mode.icon}</span><div><small>当前经营模式</small><strong>${mode.name}</strong><em>${extra}</em></div><i><b style="width:${days/336*100}%"></b></i><small>${state.gameMode==="lean"?mode.rule:state.gameMode==="story"||state.gameMode==="free"?mode.rule:`年度进度 ${days}/336 天`}</small>`;
...
function modeLogCard(icon,label,value,note){return `<article><span>${icon}</span><small>${label}</small><strong>${value}</strong><em>${note}</em></article>`}
function renderModeLogSummary(){
  const guide=document.querySelector("#modeLogGuidance"),summary=document.querySelector("#logSummary"),detailed=chapterNumber()>=3,lock=document.querySelector("#logAnalysisLock");document.querySelector(".log-guests-heading")?.toggleAttribute("hidden",!detailed);document.querySelector("#guestStats")?.toggleAttribute("hidden",!detailed);document.querySelector(".history-book")?.toggleAttribute("hidden",!detailed);if(lock)lock.hidden=detailed;if(!guide||!summary||!state.gameMode)return;ensureModeStats();const mode=GAME_MODES[state.gameMode],total=fortressTotals(),integrity=Math.round(total.health/total.max*100),weak=Object.entries(state.fortress.parts).sort(([,a],[,b])=>a.health/a.max-b.health/b.max)[0],maxed=Object.values(state.relationships).filter(rel=>rel.favor>=10).length,met=state.metCustomers.length,days=Math.min(336,state.day-state.modeStats.periodStart+1),top=Object.entries(state.modeStats.revenueByVisitor).sort((a,b)=>b[1]-a[1])[0];
  guide.innerHTML=`<span>${mode.icon}</span><div><small>本局主目标 · 第 ${days}/336 天</small><strong>${mode.name}</strong><p>${mode.rule}</p></div><b>${state.modeWon?"✓ 已赢得此模式":modeProgressSummary()}</b>`;
  if(state.gameMode==="lean")summary.innerHTML=modeLogCard("🪙","当前钱箱余额",`${state.coins} 枚`,`高分目标 · 年度收入 ${state.modeStats.revenue}/220`)+modeLogCard("👤","贡献最高的来客",top?.[0]||"暂无成交",top?`贡献 ${top[1]} 点经营收入`:"完成交易后开始统计")+modeLogCard("♟","小团队规模",`${state.employees.length} 位`,`历史峰值 ${state.modeStats.maxEmployees}/3`)+modeLogCard("🏠","当前防御状态",`${integrity}%`,`${SHOP_PARTS[weak[0]].name}最弱 · ${weak[1].health}/${weak[1].max}`);
  else if(state.gameMode==="staff"){const avg=state.employees.length?Math.round(state.employees.reduce((sum,e)=>sum+ensureEmployeeMorale(e).morale,0)/state.employees.length):0,debt=state.employees.filter(e=>e.debt).length,roles=new Set(state.employees.map(e=>e.role)).size;summary.innerHTML=modeLogCard("👥","在职员工",`${state.employees.length} / 8`,`还需 ${Math.max(0,8-state.employees.length)} 位达到人数线`)+modeLogCard("♥","团队平均士气",`${avg} / 70`,`${state.employees.filter(e=>ensureEmployeeMorale(e).morale<40).length} 位处于低士气`)+modeLogCard("✊","年度劳资记录",`${state.modeStats.strikes} 次罢工`,`${debt} 位员工仍有欠薪`)+modeLogCard("🧰","岗位与信用",`${roles} 类岗位`,`信用 ${state.survival?.credit??5}/5 · 目标至少 4`)}
  else if(state.gameMode==="storm")summary.innerHTML=modeLogCard("🌪️","本年度灾害",`${state.modeStats.disasters} / 8`,`当前预报风险 ${state.tomorrowHazard?.risk||0}%`)+modeLogCard("🏠","小店总坚固度",`${integrity}% / 80%`,`当前 ${total.health}/${total.max}`)+modeLogCard(SHOP_PARTS[weak[0]].icon,"最薄弱部件",SHOP_PARTS[weak[0]].name,`${weak[1].health}/${weak[1].max} · ${weak[1].health<=0?"已经损坏":"仍可工作"}`)+modeLogCard("🛡️","已安装加固",`${state.fortress.installed.length} 项`,`${Object.values(state.fortress.parts).filter(part=>part.health<=0).length} 个彻底损坏部件`);
  else if(state.gameMode==="story"){const first=state.metCustomers.includes(SPECIAL_NAMES[0]),second=state.metCustomers.includes(SPECIAL_NAMES[1]),next=first?SPECIAL_STORY_GUESTS[1]:SPECIAL_STORY_GUESTS[0],unlock=storyGuestUnlocked(next);summary.innerHTML=modeLogCard("📜","特殊来客",`${Number(first)+Number(second)} / 2`,first&&second?"两段隐藏故事均已抵达":`下一位：${next.name}`)+modeLogCard("🔔","已结识来客",`${met} 位`,`${next===SPECIAL_STORY_GUESTS[0]?18:26} 位为下一阶段门槛`)+modeLogCard("♥","满好感关系",`${maxed} 位`,`${next===SPECIAL_STORY_GUESTS[0]?4:8} 位为下一阶段门槛`)+modeLogCard(unlock?"🌟":"🔒","下一位前置条件",unlock?"已经满足":"尚未满足",unlock?`等待${next.schedule[0].label}`:`章节 ${chapterNumber()}/${next.chapter} · 坚固度 ${integrity}%`)}
}
const renderLogBeforeModes=renderLog;
renderLog=function(){renderLogBeforeModes();renderModeLogSummary()};
const renderBeforeModes=render;
render=function(){renderBeforeModes();ensureModeStats();renderModeUI();checkStoryVictory()};

function chooseGameMode(id){if(state.gameMode||!GAME_MODES[id])return;state.gameMode=id;state.modeStats={periodStart:state.day,revenue:0,revenueByVisitor:{},maxEmployees:state.employees.length,disasters:0,strikes:0,rolesUsed:[],victories:[]};if(id==="story")installStoryGuests();state.history.unshift({day:state.day,story:`你在新账本的扉页写下「${GAME_MODES[id].name}」。`,outcome:`经营模式确定 · ${GAME_MODES[id].rule}`});save();document.querySelector("#modeSelectDialog").close();render();if(id!=="free"&&fortressTotals().health<=0)triggerGameOver("destroyed")}
