...
function renderModeLogSummary(){
  ...
  if(state.gameMode==="lean")summary.innerHTML=modeLogCard("🪙","本年度经营收入",`${state.modeStats.revenue} / 220`,`还差 ${Math.max(0,220-state.modeStats.revenue)} 点达到收入线`)+modeLogCard("👤","贡献最高的来客",top?.[0]||"暂无成交",top?`贡献 ${top[1]} 点经营收入`:"完成交易后开始统计")+modeLogCard("♟","小团队规模",`${state.employees.length} 位`,`全年峰值 ${state.modeStats.maxEmployees}/3`)+modeLogCard("🏠","年末防御要求",`${integrity}% / 80%`,`${SHOP_PARTS[weak[0]].name}最弱 · ${weak[1].health}/${weak[1].max}`);
