...
function autoplayPolicy(){return window.LateLanternAILab?.policy?.()||{openCoverage:.42,crisisCoverage:.82,hireCoinBuffer:8,hireWeeks:1,repairNormal:.82,repairCrisis:.5,recruitDecorBonus:8}}
...
function hireReadiness(visitor){const cfg=EMPLOYMENT[visitor.name],policy=autoplayPolicy(),target=Math.min(autoplayHiringTarget(),typeof modeEmployeeCap==="function"?modeEmployeeCap():Infinity);if(!cfg)return {ok:false,reason:"缺少雇佣配置"};if(state.employees.length>=target)return {ok:false,reason:`当前模式的主动招聘目标已达 ${target} 人`};if(totalDeals()<4)return {ok:false,reason:"先完成 4 笔交易建立经营基础"};if((state.survival?.credit??5)<3)return {ok:false,reason:"信用不足，先恢复信用"};if((state.survival?.noTradeDays||0)>=4)return {ok:false,reason:"正处于断供危机，先恢复成交"};const wage=cfg.wage,weeks=policy.hireWeeks||1,ok=wage.type==="coins"?state.coins>=wageReserve("coins")+wage.amount*weeks+policy.hireCoinBuffer:count(wage.item)>=wageReserve("item",wage.item)+wage.amount*weeks+1;return {ok,reason:ok?"已备好首周工资与应急储备":`尚未备足首周${wage.type==="coins"?"铜币工资":wage.item+"工资"}`}}
