...
function autoplayStaffAction(){
  ...
  const demand=desiredMaterials();for(const employee of workers.filter(e=>e.role==="gather")){const locations=EMPLOYMENT[employee.name].abilities.gather.locations,cfg=typeof employeeWorkConfig==="function"?employeeWorkConfig(employee.name):{likes:[],dislikes:[]},bestLocation=locations.map(id=>({id,score:gatherPool(id).reduce((sum,rule)=>sum+(demand.get(rule.item)||.2)+(rule.item===EMPLOYMENT[employee.name].abilities.gather.specialty?2:0),0)+workers.filter(other=>other!==employee&&other.role==="gather"&&(other.gatherLocation||EMPLOYMENT[other.name].abilities.gather.locations[0])===id).reduce((score,other)=>{const otherCfg=employeeWorkConfig(other.name);return score+(cfg.likes.includes(other.name)?3:0)+(otherCfg.likes.includes(employee.name)?3:0)-(cfg.dislikes.includes(other.name)?10:0)-(otherCfg.dislikes.includes(employee.name)?10:0)},0)})).sort((a,b)=>b.score-a.score)[0]?.id;if(bestLocation&&bestLocation!==(employee.gatherLocation||locations[0])){const thought=`兼顾材料需求与同事关系，调整${employee.name}到${bestLocation}`;if(window.LateLanternAILab?.fast){employee.gatherLocation=bestLocation;return {direct:true,thought}}const select=document.querySelector(`[data-gather-location="${employee.name}"]`);if(select)return {select,role:bestLocation,thought}}}
  return null
}
